...
import officefurnitures from "../../data/officefurnitures";
...
class ProductDetails extends Component {
  ...
  componentDidMount() {
    window.scrollTo(0, 0);
    this.setState({ officefurnitures });
  }
  render() {
    const id = this.props.match.params.id;
    const theproduct = this.state.officefurnitures.find(
      (product) => product.id.toString() === id
    );

    const categories = this.state.officefurnitures.filter(
      (product) => product.category === theproduct.category
    );

    console.log(categories);
    if (this.state.officefurnitures.length === 0) {
      return <Spinner />;
    }
    return (
      <>
