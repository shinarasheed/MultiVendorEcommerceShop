...
class ProductDetails extends Component {
  ...
  componentDidMount() {
    window.scrollTo(0, 0);
    this.setState({ officeequipments });
  }
  render() {
    const id = this.props.match.params.id;
    const theproduct = this.state.officeequipments.find(
      (product) => product.id.toString() === id
    );

    const categories = this.state.officeequipments.filter(
      (product) => product.category === theproduct.category
    );

    console.log(categories);
    if (this.state.officeequipments.length === 0) {
      return <Spinner />;
    }
    return (
      <>
