...
import medicalequipments from "../../data/medicalequipments";
...
class ProductDetails extends Component {
  ...
  componentDidMount() {
    window.scrollTo(0, 0);
    this.setState({ medicalequipments });
  }
  render() {
    const id = this.props.match.params.id;
    const theproduct = this.state.medicalequipments.find(
      (product) => product.id.toString() === id
    );

    const categories = this.state.medicalequipments.filter(
      (product) => product.category === theproduct.category
    );

    console.log(categories);
    if (this.state.medicalequipments.length === 0) {
      return <Spinner />;
    }
    return (
      <>
